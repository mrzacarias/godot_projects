...
class TouchMovement extends MovementStrategy:
	func calculate_movement(player: CharacterBody2D, _delta: float) -> Vector2:
		...
		# Calculate direction and apply speed with multiplier
		var direction = (player.target_position - player.global_position).normalized()
		var distance_factor = min(distance_to_target / 200.0, 1.0)  # Slow down when close
		
		return direction * player.speed * player.touch_speed_multiplier * distance_factor * player.speed_mult
...
class MovementManager:
	...
	func apply_camera_bounds_constraint(player: CharacterBody2D, delta: float):
		...
		if camera_bounds_cache_timer >= camera_bounds_cache_duration or cached_camera_bounds == Rect2():
			if player.has_method("get_camera_bounds"):
				...
				cached_camera_bounds = Rect2(Vector2.ZERO, Vector2(2880, 1920))
	...
	func update_player_visuals(player: CharacterBody2D, velocity: Vector2):
		"""Update player sprite and animations based on movement"""
		if not player.player_character:
			return
		
		# Update movement direction for attacks and animations
		if velocity != Vector2.ZERO:
			player.last_movement_direction = velocity.normalized()
		...
		# Handle animations
		if player.player_character.has_method("play_walk_animation") and player.player_character.has_method("play_idle_animation"):
