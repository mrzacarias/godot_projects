...
# Vamlike Survivors Pack and Deploy Script
...
echo "🚀 Starting Vamlike Survivors pack and deploy..."
...
# Use existing html directory (don't overwrite)
OUTPUT_DIR="html"
...
"$GODOT_PATH" --headless --export-release "Web" "$OUTPUT_DIR/index.html" 2>&1 | tee /tmp/godot_export_vamlike.log
...
if [ $EXPORT_EXIT_CODE -eq 0 ]; then
    ...
    if [ -f "$OUTPUT_DIR/index.html" ] && [ -f "$OUTPUT_DIR/index.js" ] && [ -f "$OUTPUT_DIR/index.wasm" ]; then
        ...
        # Create compressed archive in the current directory
        echo "📦 Creating compressed archive..."
        ARCHIVE_NAME="vamlike-survivors-html.zip"
        ...
        if [ -f "$ARCHIVE_NAME" ]; then
            ...
            if ! command -v butler &> /dev/null; then
                ...
                echo "📤 Uploading to zacasoft/vamlike-survivors:web..."
                butler push "$ARCHIVE_NAME" zacasoft/vamlike-survivors:web --userversion "$(date +%Y.%m.%d.%H%M)"
                
                if [ $? -eq 0 ]; then
                    echo "✅ Successfully deployed to itch.io!"
                    echo "🌐 Game updated at: https://zacasoft.itch.io/vamlike-survivors"
                else
        ...
        echo "📋 Export log:"
        cat /tmp/godot_export_vamlike.log
        exit 1
    fi
else
