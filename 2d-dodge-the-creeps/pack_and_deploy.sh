...
# Dodge the Creeps Pack and Deploy Script
...
echo "🚀 Starting Dodge the Creeps pack and deploy..."
...
"$GODOT_PATH" --headless --export-release "Web" "$OUTPUT_DIR/index.html" 2>&1 | tee /tmp/godot_export_dodge.log
...
if [ $EXPORT_EXIT_CODE -eq 0 ]; then
    ...
    if [ -f "$OUTPUT_DIR/index.html" ] && [ -f "$OUTPUT_DIR/index.js" ] && [ -f "$OUTPUT_DIR/index.wasm" ]; then
        ...
        ARCHIVE_NAME="dodge-the-creeps-html.zip"
        ...
        if [ -f "$ARCHIVE_NAME" ]; then
            ...
            if ! command -v butler &> /dev/null; then
                ...
                echo "📤 Uploading to zacasoft/dodge-the-creeps:web..."
                butler push "$ARCHIVE_NAME" zacasoft/dodge-the-creeps:web --userversion "$(date +%Y.%m.%d.%H%M)"
                
                if [ $? -eq 0 ]; then
                    echo "✅ Successfully deployed to itch.io!"
                    echo "🌐 Game updated at: https://zacasoft.itch.io/dodge-the-creeps"
                else
        ...
        echo "📋 Export log:"
        cat /tmp/godot_export_dodge.log
        exit 1
    fi
else
